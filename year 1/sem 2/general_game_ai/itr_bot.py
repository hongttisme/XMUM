...
class HongSABot:
    ...
    def play(self, the_game_gird, stack):
        ...
        if self.mode == "aim_base":
            ...
            if self.turn > 200 and len(self.lowest_tower) > 0:
                self.mode = "take_tower"
                aiming_y = self.lowest_tower[1][0]
                aiming_x = self.lowest_tower[1][1]

        print(self.mode, self.turn)
...
def lighten(colour):
    a = 30
    r = min(colour[0] + a, 255)
    g = min(colour[1] + a, 255)
    b = min(colour[2] + a, 255)
    new_colour = (r, g, b)
    return new_colour
...
def total_army(total_red_army, total_blue_army):
    for i in range(n):
        for j in range(n):
            if game_grid[i][j].territory == Square.RED:
                total_red_army += game_grid[i][j].value
            elif game_grid[i][j].territory == Square.BLUE:
                total_blue_army += game_grid[i][j].value

    # print(f"Red:{total_red_army}")
    # print(f"Blue:{total_blue_army}")
    return [total_red_army, total_blue_army]


def total_land(total_red_land, total_blue_land):
    for i in range(n):
        for j in range(n):
            if game_grid[i][j].territory == Square.RED and game_grid[i][j].type == "land":
                total_red_land += 1
            elif game_grid[i][j].territory == Square.BLUE and game_grid[i][j].type == "land":
                total_blue_land += 1
    # print(f"Red land:{total_red_land}")
    # print(f"Blue land:{total_blue_land}")
    return [total_red_land, total_blue_land]
...
testing_time = 10
result_list = []
while testing_time > 0:
    testing_time -= 1
    # adjustable variables initialization
    game_width = 760  # screen is always square, so no need game height
    n = 20
    land_army_growth_interval = 25  # 25
    base_army_growth_interval = 1  # 1
    cooldown = 0.5  # 0.3
    game_grid = np.array([[Square] * n] * n)
    ...
    while True:
        total_of_red_army = 0
        total_of_blue_army = 0
        total_of_red_land = 1
        total_of_blue_land = 1

        total_army(total_of_red_army, total_of_blue_army)
        total_land(total_of_red_land, total_of_blue_land)

        red_base = False
        blue_base = False
        fog_grid = np.array([[0] * n] * n)
        time = (pygame.time.get_ticks() - start_time) / 1000  # record time
        bot1.play(game_grid, blue_move_queue)
        ...
        for i in range(n):  # intialize fog
            for j in range(n):
        ...
        for i in range(n):  # base graphics
            for j in range(n):
        ...
        if not (blue_base and red_base):  # check if the game is ended
            running = False
        ...
        # pygame.draw.rect(game_display, darken(game_grid[selected_y][selected_x].territory),
        #     [, selected_y * square_width, square_width, square_width])
        ...
        if time - blue_timer >= cooldown and len(blue_move_queue) > 0:
            ...
            if check_legal_move(game_grid, Square.BLUE, move) == False:
                ...
                if played_turn > 120:
                    pygame.quit()
                    result_list.append([total_army(total_of_red_army, total_of_blue_army),
                                        total_land(total_of_red_land, total_of_blue_land)])
                    print([total_army(total_of_red_army, total_of_blue_army),
                                        total_land(total_of_red_land, total_of_blue_land)])
                    break
